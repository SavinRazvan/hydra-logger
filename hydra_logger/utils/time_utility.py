...
class TimestampConfig:
    ...
    def format_timestamp(self, dt: datetime) -> str:
        ...
        return TimestampFormatter.format_timestamp(
            dt, self.format_type, self.precision, self.timezone_name
        )

    def get_current_timestamp(self) -> str:
        """Get current timestamp using this configuration."""
        return TimestampFormatter.get_current_timestamp(
            self.format_type, self.precision, self.timezone_name
        )
...
class TimeUtility:
    """Time utility class for Hydra-Logger.
...
class TimestampFormatter:
    """Timestamp formatter following industry standards.
    ...
    def _format_unix_timestamp(
        dt: datetime, format_type: TimestampFormat, precision: TimestampPrecision
    ) -> str:
    ...
    def get_current_timestamp(
        ...
        return TimestampFormatter.format_timestamp(
            now, format_type, precision, timezone_name
        )

    @staticmethod
    def parse_timestamp(
        timestamp_str: str, format_type: TimestampFormat = TimestampFormat.RFC3339
    ) -> datetime:
    ...
    def _parse_unix_timestamp(
        timestamp_str: str, format_type: TimestampFormat
    ) -> datetime:
