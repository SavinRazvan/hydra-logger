...
class BaseLogger(ABC):
    ...
    def __init__(self, config: Optional[LoggingConfig] = None, **kwargs):
        ...
        # ✅ STANDARDIZED: LogRecord creation strategy
    ...
    def _setup_record_creation_strategy(self) -> None:
        ...
        try:
            ...
            self._record_creation_strategy = get_record_creation_strategy(self._performance_profile)
        except ImportError:
            # Fallback if record_creation module is not available
            self._record_creation_strategy = None
    
    def create_log_record(self, level: Union[str, int], message: str, **kwargs) -> LogRecord:
        """
        ...
        if self._record_creation_strategy:
            ...
            return LogRecord(
                timestamp=time.time(),
                level_name=str(level) if isinstance(level, str) else self._get_level_name(level),
    ...
    def get_record_creation_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'strategy': self._record_creation_strategy.strategy if self._record_creation_strategy else 'fallback',
