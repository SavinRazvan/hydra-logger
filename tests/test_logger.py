...
"""
        ...
        with pytest.raises(HydraLoggerError, match="Failed to create HydraLogger from config"):
            HydraLogger.from_config(config_file)
            ...
            layers={
                "TEST": LogLayer(
                    destinations=[
                        LogDestination(
                            type="file",
                            path=os.path.join(readonly_dir, "test.log")
                        )
                    ]
                )
            }
    ...
    def test_create_file_handler_missing_path(self):
            ...
            layers={
                "TEST": LogLayer(
                    destinations=[
                        LogDestination(
                            type="file",
                            path="logs/test.log"  # Valid path for config
                        )
                    ]
                )
            }
        ...
        with patch.object(logger, '_create_file_handler') as mock_handler:
            mock_handler.side_effect = ValueError("Path is required for file destinations")
            ...
            layers={
                "TEST": LogLayer(
                    destinations=[
                        LogDestination(
                            type="console",
                            level="INFO"  # Valid level for config
                        )
                    ]
                )
            }
    ...
    def test_initialization_with_exception(self):
        ...
        with patch('hydra_logger.logger.get_default_config', side_effect=Exception("Config error")):
            with pytest.raises(HydraLoggerError, match="Failed to initialize HydraLogger"):
                HydraLogger()
    ...
    def test_setup_loggers_with_exception(self):
        ...
        with patch('hydra_logger.logger.create_log_directories', side_effect=Exception("Directory error")):
    ...
    def test_setup_single_layer_with_exception(self):
        ...
        with patch.object(logger, '_create_handler', side_effect=Exception("Handler error")):
    ...
    def test_create_handler_validation_error(self):
        ...
        with patch.object(logger, '_create_file_handler', side_effect=ValueError("Validation error")):
    ...
    def test_create_handler_fallback_to_console(self):
        ...
        with patch.object(logger, '_create_file_handler', side_effect=Exception("File error")):
    ...
    def test_create_handler_fallback_console_fails(self):
        ...
        with patch.object(logger, '_create_file_handler', side_effect=Exception("File error")):
            with patch.object(logger, '_create_console_handler', side_effect=Exception("Console error")):
                result = logger._create_handler(destination, "INFO")
                assert result is None
    ...
    def test_create_file_handler_os_error(self):
        ...
        with patch('hydra_logger.logger.RotatingFileHandler', side_effect=OSError("File system error")):
