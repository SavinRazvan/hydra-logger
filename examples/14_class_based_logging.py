#!/usr/bin/env python3
"""
Role: Runnable example for 14 class based logging.
Used By:
    ...
    layers={
        "app": LogLayer(
            destinations=[
                ...
                LogDestination(
                    type="file",
                    path="logs/examples/14_class_based_logging_app.jsonl",
                    format="json-lines",
                ),
            ]
        ),
        "service": LogLayer(
            destinations=[
                LogDestination(
                    type="file",
                    path="logs/examples/14_class_based_logging_service.log",
                    format="json",
                )
            ]
        ),
        "model": LogLayer(
            destinations=[
                LogDestination(
                    type="file",
                    path="logs/examples/14_class_based_logging_model.log",
                    format="json",
                )
            ]
        ),
...
class UserModel:
    ...
    def __init__(self, user_id: int, username: str, email: str):
        ...
        self.email = email
        logger.debug(
            f"[14] User model created: {username} (ID: {user_id})", layer="model"
        )
    ...
    def update_email(self, new_email: str):
        """Update user email."""
        logger.info(
            f"[14] Updating email for user {self.user_id}: {new_email}", layer="model"
        )
...
def main():
    ...
    user1.to_dict()
    UserModel.from_dict({"id": 2, "username": "bob", "email": "bob@example.com"})
