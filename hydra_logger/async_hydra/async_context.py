...
class AsyncContextManager:
    ...
    async def __aenter__(self):
        ...
        try:
            ...
            # Ensure we don't leave context in an inconsistent state
            print(f"Error entering async context: {e}", file=sys.stderr)
            raise
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        try:
            ...
            print(f"Error exiting async context: {e}", file=sys.stderr)
    ...
    def set_current_context(cls, context: AsyncContext) -> None:
        ...
        try:
            ...
            print(f"Error setting async context: {e}", file=sys.stderr)
    ...
    def clear_current_context(cls) -> None:
        ...
        try:
            ...
            print(f"Error clearing async context: {e}", file=sys.stderr)
...
class AsyncTraceManager:
    ...
    def start_trace(self, trace_id: Optional[str] = None) -> str:
        ...
        try:
            ...
            print(f"Error starting trace: {e}", file=sys.stderr)
    ...
    def set_correlation_id(self, correlation_id: str) -> None:
        ...
        try:
            ...
            print(f"Error setting correlation ID: {e}", file=sys.stderr)
    ...
    def clear_trace(self) -> None:
        ...
        try:
            ...
            print(f"Error clearing trace: {e}", file=sys.stderr)
...
class AsyncContextSwitcher:
    ...
    def detect_context_switch(self, current_context: Optional[AsyncContext]) -> bool:
        ...
        try:
            ...
            print(f"Error detecting context switch: {e}", file=sys.stderr)
            return False
...
def get_trace_id() -> Optional[str]:
    """Get current trace ID."""
    return _trace_manager.get_trace_id()


def start_trace(trace_id: Optional[str] = None) -> str:
    """Start a new trace."""
    return _trace_manager.start_trace(trace_id)


def set_correlation_id(correlation_id: str) -> None:
    """Set correlation ID for current trace."""
    _trace_manager.set_correlation_id(correlation_id)


def get_correlation_id() -> Optional[str]:
    """Get current correlation ID."""
    return _trace_manager.get_correlation_id()


def detect_context_switch(context: Optional[AsyncContext]) -> bool:
    """Detect if a context switch has occurred."""
    return _context_switcher.detect_context_switch(context)
...
async def async_context(context: Optional[AsyncContext] = None):
    """
    Async context manager for preserving async context.
    
    Args:
        context (Optional[AsyncContext]): Context to use or None for auto-generated
        
    Example:
        >>> async with async_context():
        ...     await logger.info("LAYER", "Message with context")
    """
...
async def trace_context(trace_id: Optional[str] = None):
    ...
    Example:
        >>> async with trace_context("request-123"):
        ...     await logger.info("LAYER", "Message with trace")
    """
    try:
