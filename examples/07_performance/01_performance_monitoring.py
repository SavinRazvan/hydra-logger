...
def demo_performance_monitoring():
    ...
    logger.info("PERF", "Application started", extra={"startup_time_ms": 150, "memory_usage_mb": 256, "cpu_percent": 15})
    
    logger.info("PERF", "Database query executed", extra={"query_time_ms": 45, "rows_returned": 1000, "cache_hit_rate": 0.85})
    
    logger.info("PERF", "API response time", extra={"response_time_ms": 120, "status_code": 200, "payload_size_kb": 2.5})
    
    logger.info("PERF", "Memory usage update", extra={"memory_usage_mb": 512, "memory_peak_mb": 768, "gc_collections": 3})
    ...
    for i in range(100):
        logger.info("PERF", f"Performance test message {i+1}", extra={"message_id": i+1, "timestamp": time.time()})
        log_count += 1
    ...
    def log_messages(thread_id):
        ...
        for i in range(20):
            logger.info("PERF", f"Concurrent message from thread {thread_id}", extra={"thread_id": thread_id, "message_id": i+1, "timestamp": time.time()})
