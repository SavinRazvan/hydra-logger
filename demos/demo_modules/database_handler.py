...
class DatabaseHandler:
    ...
    def execute_query(self, query: str, params: Dict = None) -> List[Dict]:
        ...
        try:
            ...
            self.logger.info("DATABASE", f"Query executed successfully, returned {len(results)} rows")
            return results
        except Exception as e:
    ...
    def save_user_data(self, user_data: Dict[str, Any]) -> bool:
        ...
        self.logger.info("DATABASE", f"Saving user data for user ID: {user_data.get('id')}")
        try:
            if not self.connected:
                raise Exception("Database not connected")
            time.sleep(0.15)
            self.logger.info("DATABASE", f"User data saved successfully for user ID: {user_data.get('id')}")
            return True
        except Exception as e:
