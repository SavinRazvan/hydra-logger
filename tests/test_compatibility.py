...
class TestSetupLogging:
    ...
    def test_setup_logging_file_only(self, temp_dir):
        ...
        assert isinstance(handlers[0], RotatingFileHandler), "Should have RotatingFileHandler"
    ...
    def test_setup_logging_console_only(self, temp_dir):
        ...
        assert isinstance(handlers[0], logging.StreamHandler), "Should have StreamHandler"
        
        # Logs directory should not be created
        logs_dir = os.path.join(temp_dir, "logs")
        assert not os.path.exists(logs_dir), "Logs directory should not be created when file logging is disabled"
    ...
    def test_setup_logging_custom_levels(self, temp_dir):
        ...
        for handler in logger.handlers:
            if isinstance(handler, RotatingFileHandler):
                assert handler.level == logging.DEBUG, "File handler should be set to DEBUG"
            elif isinstance(handler, logging.StreamHandler):
                assert handler.level == logging.ERROR, "Console handler should be set to ERROR"
    ...
    def test_setup_logging_directory_creation_failure(self, temp_dir):
        ...
        assert len(logger.handlers) == 1, "Should have console handler even if file handler fails"
        assert isinstance(logger.handlers[0], logging.StreamHandler), "Should have StreamHandler"
...
class TestMigrationFunctions:
    ...
    def test_create_hydra_config_from_legacy(self):
        ...
        file_dest = next(d for d in config.layers["DEFAULT"].destinations if d.type == "file")
        ...
        console_dest = next(d for d in config.layers["DEFAULT"].destinations if d.type == "console")
