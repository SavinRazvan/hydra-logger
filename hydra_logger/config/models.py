...
class LogDestination(BaseModel):
    ...
    Attributes:
        ...
        format (str): Log format: 'plain-text', 'json', 'csv', 'syslog', or 'gelf' (default: 'plain-text').
        color_mode: Literal["auto", "always", "never"] = Field(
            default="auto", description="Color mode for this destination"
        )
    ...
    format: str = Field(
        default="plain-text",
        description=("Log format: 'plain-text' (human-readable), 'json', 'csv', 'syslog', or 'gelf'. Use color_mode to control colors for any format."),
    )
    color_mode: Literal["auto", "always", "never"] = Field(
        default="auto", description="Color mode for this destination"
    )
    ...
    def validate_format(cls, v: str) -> str:
        ...
        if v.lower() not in valid_formats:
            error_msg = (
                f"Invalid format '{v}'. Valid formats are: {', '.join(valid_formats)}. "
                f"Use color_mode parameter to control colors (auto/always/never)."
            )
...
def get_default_config() -> LoggingConfig:
    ...
    Returns:
        ...
        - Console destination: DEBUG level output
        - Default level: DEBUG for all layers
