...
class Utils:
    ...
    def generate_user_id(self, username: str, email: str) -> str:
        ...
        try:
            ...
            self.logger.debug(
                "UTILS", f"Generated user ID: {user_id} for user: {username}"
            )
            return user_id
        except Exception as e:
            self.logger.error(
                "UTILS", f"Error generating user ID for {username}: {str(e)}"
            )
            raise
    ...
    def format_user_data(self, user_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.debug(
                "UTILS",
                f"User data formatted successfully for user ID: {formatted_data.get('id')}",
            )
    ...
    def create_audit_log(self, action: str, user_id: int, details: Optional[Dict] = None) -> Dict:
        ...
        try:
            ...
            self.logger.info(
                "UTILS", f"Audit log created successfully for action: {action}"
            )
            return audit_entry
        except Exception as e:
            self.logger.error(
                "UTILS", f"Error creating audit log for action {action}: {str(e)}"
            )
            raise
