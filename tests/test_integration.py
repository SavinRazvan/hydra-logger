...
class TestIntegration:
    ...
    def test_multi_module_logging_with_custom_paths(self, temp_dir):
            ...
            layers={
                ...
                "DB": LogLayer(
                    ...
                    destinations=[
                        LogDestination(
                            type="file",
                            path=os.path.join(temp_dir, "logs", "database", "queries.log"),
                            max_size="5MB",
                            backup_count=3
                        )
                    ]
        ...
        logger.critical("AUTH", "Security breach detected: multiple failed login attempts")
        ...
        for log_file, expected_messages in log_files_to_check:
            ...
            with open(log_file, 'r') as f:
                content = f.read()
                print(f"📄 {os.path.basename(log_file)}: {len(content.splitlines())} lines")
                
                for message in expected_messages:
                    assert message in content, f"Message '{message}' not found in {log_file}"
            ...
            layers={
                ...
                "ERROR_LAYER": LogLayer(
                    ...
                    destinations=[
                        LogDestination(
                            type="file",
                            path=os.path.join(temp_dir, "logs", "error", "critical.log")
                        )
                    ]
                )
            }
